...
This analysis compares the resolved upstream ARM64 baseline with NVIDIA Jetson
AGX Orin/Tegra234. It is not a compatibility claim or a port design. No Orin
hardware was flashed or tested.
...
* **Experimentally tested** is limited to the ZynqMP QEMU baseline in Task 001.
...
`arch/arm64/boot/dts/nvidia/tegra234*.dts*`.
...
* **Documented behavior:** NVIDIA's Jetson boot architecture identifies a T23x
  ...
  requires T234 MB1 BCT and device-tree adaptation. Linux describes Tegra234
  Cortex-A78 CPUs with PSCI enablement.
* **Experimentally tested:** no Orin test. The ZynqMP QEMU image built, but boot
  was blocked before emulator startup by a missing host `guestfs` prerequisite.
...
* **Experimentally tested:** no runtime test occurred. GICv2 behavior is specified
  by the ZynqMP QEMU CI expectations; no GICv3 or Orin test occurred.
...
* **Experimentally tested:** image build only; no timer runtime or Orin measurement.
...
* **Experimentally tested:** Cortex-A53/ZynqMP only; no Cortex-A78/Orin test.
...
* **Experimentally tested:** ZynqMP policy only; no Orin DMA or fault-containment
  experiment.
...
* **Documented behavior:** current ZynqMP policy is four Cortex-A53 cores. Linux
  v6.1 Tegra234 CPU nodes are Cortex-A78 and use PSCI; MPIDR topology differs.
* **Experimentally tested:** the selected *minimal* baseline is one-core; it does
  not test the separate multicore recipe. No Orin test occurred.
...
* **Documented behavior:** NVIDIA identifies J26 USB Micro-B as the developer-kit
  debug UART and J502 as JTAG. Linux's developer-kit DTS aliases `serial0` to the
  mailbox-backed Tegra Combined UART (`nvidia,tegra234-tcu`) and `serial1` to
  UARTA (`nvidia,tegra234-uart` at `0x03100000`).
* **Experimentally tested:** no Orin console test.
...
* **Experimentally tested:** the existing ZynqMP QEMU image built, but did not
  start because of the host launcher prerequisite; no Orin guest, NVIDIA driver,
  GPU, or CUDA test.
...
* **Inspected implementation:** the separate proof recipe runs GNATprove on the
  Ada `sk-kernel` main in `BUILD_MODE=prove`. At this revision, `src/main` has 78
  Ada units, ten assembly files, and 19 Ada files containing explicit
  `SPARK_Mode => Off`. The latter include system-register access, CPU/FPU/MMU/
  SLAT/timer operations, SMMU barriers, atomics, locks, barriers, power, and
  architecture glue. Startup, EL3 GIC/SMMU setup, exception vectors, cache code,
  and subject entry are assembly and outside Ada proof.
* **Documented behavior:** `classes/muen_proof.yaml` uses GNATprove
  ...
  proof. This proves obligations in the analyzed SPARK subset, not hardware
  compatibility or assembly correctness.
* **Experimentally tested:** no proof command was run in Task 001; the debug
  baseline provides no proof result.
* **Unknown:** proof obligations and trusted-code growth for future GICv3,
  Cortex-A78, boot, console, SMMU-topology, and policy changes. A port must define
...
architecture offer potential reuse, but boot/firmware handoff, Cortex-A78 CPU
